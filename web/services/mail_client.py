...
def _read_env_bool(key: str, default: bool = False) -> bool:
    """Read a boolean-ish key from process env, then the mounted host .env file."""
...
def _read_env_value(key: str, default: str = "") -> str:
    """Read a string key from process env, then the mounted host .env file."""
    raw = os.environ.get(key)
    if raw is not None:
        ...
        *,
        ...
        resp = requests.post(
            ...
            headers={
                ...
                "User-Agent": "TAKNET-PS-AIS-Aggregator",
